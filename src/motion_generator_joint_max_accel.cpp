...
franka_joint_motion_generator::franka_joint_motion_generator(double speed_factor,
    const std::array<double, 7> q_goal,
    bool stop_on_contact)
    : q_goal_(q_goal.data()), stop_on_contact_(stop_on_contact)
{
...
bool franka_joint_motion_generator::calculateDesiredValues(double t, Vector7d *delta_q_d) const
{
    ...
    return std::all_of(
        joint_motion_finished.cbegin(), joint_motion_finished.cend(), [](bool x) { return x; });
}
    ...
    franka::Duration period)
{
    ...
    if (stop_on_contact_ && colliding(robot_state)) throw contact_stop_trigger();
