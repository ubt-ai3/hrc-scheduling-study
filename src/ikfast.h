...
/** \brief  Header file for all ikfast c++ files/shared objects.
   ...
   precision/settings that the library was compiled with The defines are as follows, they are also
   used for the ikfast C++ class:
...
/// if 0x10000000 bit is set, then the iksolver assumes 6D transforms are done without the
/// manipulator offset taken into account (allows to reuse IK when manipulator offset changes)
...
template<typename T> class IkSolutionListBase
{
  public:
    ...
    /// solution that users can freely set. The indices are of the configuration that the IK solver
    /// accepts rather than the entire robot, ie 0 points to the first value accepted.
...
template<typename T> class IkFastFunctions
{
  public:
    IkFastFunctions()
        : _ComputeIk(NULL), _ComputeIk2(NULL), _ComputeFk(NULL), _GetNumFreeParameters(NULL),
          _GetFreeParameters(NULL), _GetNumJoints(NULL), _GetIkRealSize(NULL),
          _GetIkFastVersion(NULL), _GetIkType(NULL), _GetKinematicsHash(NULL)
    {}
    ...
    virtual void GetSolution(T *solution, const T *freevalues) const
    {
        for (std::size_t i = 0; i < _vbasesol.size(); ++i) {
            if (_vbasesol[i].freeind < 0)
                ...
                solution[i] =
                    freevalues[_vbasesol[i].freeind] * _vbasesol[i].fmul + _vbasesol[i].foffset;
